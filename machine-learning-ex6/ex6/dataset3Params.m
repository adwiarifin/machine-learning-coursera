...
for C_test = [0.01 0.03 0.1 0.3 1 3 10 30]
	for sigma_test = [0.01 0.03 0.1 0.3 1 3 10 30]
		fprintf('Train with C=%.2f sigma=%.2f\n', C_test, sigma_test);
		model = svmTrain(X, y, C_test, @(x1, x2) gaussianKernel(x1, x2, sigma_test)); 
		predictions = svmPredict(model, Xval);
		prediction_error = mean(double(predictions ~= yval));
		fprintf('Got error = %.6f\n\n\n', prediction_error);
...
[min, minIndex] = min(results(:,3));
C = results(minIndex, 1);
sigma = results(minIndex, 2);
fprintf('Got minimum error = %.6f in the index = %d\n', min, minIndex);
fprintf('Computed C = %.2f, sigma = %.2f\n', C, sigma);
